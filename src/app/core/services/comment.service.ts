  ...
  getPostComments(
    ...
    return from(
      ...
      switchMap(({ data, error }) => {
        ...
        // Map the data to Comment objects
        const topLevelComments = data.map((comment) =>
          this.mapComment(comment)
        );

        // If we don't need replies, return just the top-level comments
        if (!includeReplies) {
          return of(topLevelComments);
        }

        // Otherwise, fetch replies for each comment
        const commentWithRepliesObservables = topLevelComments.map((comment) =>
          this.getCommentReplies(comment.id).pipe(
            map((replies) => {
        ...
        return forkJoin(commentWithRepliesObservables);
      }),
      catchError((error) => {
